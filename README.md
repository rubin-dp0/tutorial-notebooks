...
|***07. *(time-domain tutorial)*** (TBD)| 
| **08a. Interactive Image Visualization**<br> **Description:** Use Holoviews to display an image and overlay detected sources. Use DynamicMap to interact with image data. <br> **Level:** Intermediate <br> **Packages:** numpy, bokeh, holoviews<br> **Skills:** Define exposure and retrieving image; define deep coadd and retrieving image; visualizing exposure images with Holoviews; overlaying source detections on an image; exploring the image with Holoviews Streams and DynamicMap.<br>  | 
| **08b. Interactive Catalog Visualization**<br> **Description:** Use Holoviews and Bokeh to display and interact with catalog data. Use Datashader to visualize very large datasets interactively. <br> **Level:** Intermediate <br> **Packages:** numpy, pandas, bokeh, holoviews, datashader<br> **Skills:** Preparing DP0.1 tabular dataset; querying using SkyCoord; defining reference position; using data assertions; visualizing tabular data with HoloViews; creating layout of several plots, creating scatter plot; creating color-color scatter plot of the stars in the dataset; using the hover tool; linking plots and data brushing with Boke; creating color-magnitude diagram linked to a Skyplot; analyzing data with Holoviews linked streams; visualizing larger datasets with Datashader; creating a color-color plot; creating a callback function to retrieve the indicies of selected points; linking plots with interactive selection.<br> |
| **09. Single Star Lightcurve with Butler**<br> **Description:** Demonstrate limits of building light curves during DP0.1: use the Butler to retrieve all PVI sources within a small radius of a given sky coordinate. <br> **Level:** Beginner-Intermediate <br> **Packages:** lsst.daf.butler, lsst.geom, lsst.sphgeom, lsst.daf.base, matplotlib.pyplot, numpy, astropy.units, time <br> **Skills:** Using TAP Service to choose a variable star; executing spatial search of `src` catalogs with the butler; looping over search results and store `src` data; plotting a time series (magnitude vs. time); finding a star's period and make a phase-folded light curve.<br>|
